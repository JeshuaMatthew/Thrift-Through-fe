...
const CommunityForm: React.FC<CommunityFormProps> = ({ isOpen, onClose, onSuccess, initialData }) => {
  const { user } = useAuth();
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>, field: 'profilepicturl' | 'bannerurl') => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (initialData) {
        // Edit Mode
        const result = await commService.updateCommunity(initialData.communityid, user.userid, payload);
        if (result.success) {
          onSuccess();
          onClose();
        } else {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      >
        <div className="p-6 border-b border-white/10 flex justify-between items-center bg-bg-vermillion text-tx-primary">
        ...
        <div className="overflow-hidden grow flex flex-col min-h-0 bg-bg-vermillion">
          {errorMsg && (
            <div className="m-6 p-4 bg-red-500/20 text-red-100 rounded-xl text-sm border border-red-500/30 flex items-center gap-2">
              <X className="h-5 w-5 shrink-0" />
              {errorMsg}
            </div>
          )}

          <form id="community-form" onSubmit={handleSubmit} className="h-full">
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-0 h-full">
              {/* Kolom Kiri: Map */}
              <div className="relative h-[400px] lg:h-full bg-slate-900 overflow-hidden">
                <div className="absolute inset-0 z-10 p-4 pointer-events-none">
                  <div className="bg-bg-vermillion/90 backdrop-blur-md p-3 rounded-xl border border-white/10 shadow-lg inline-block pointer-events-auto">
                    <h4 className="text-xs font-gasoek font-normal tracking-wide text-tx-primary uppercase mb-1">Lokasi Komunitas *</h4>
                    <p className="text-[10px] text-tx-primary/60 font-medium font-questrial italic">Klik pada peta untuk lokasi titik temu.</p>
                  </div>
                </div>
                ...
                <div className="absolute bottom-4 right-4 z-10 flex gap-2">
                   <div className="bg-bg-clean/90 backdrop-blur-md px-3 py-1.5 rounded-lg border border-white/10 shadow-sm">
                      <span className="text-[10px] font-bold text-tx-primary/40 mr-1">LAT</span>
                      <span className="text-[10px] font-medium text-tx-primary font-questrial">{Number(formData.latitude).toFixed(4)}</span>
                   </div>
                   <div className="bg-bg-clean/90 backdrop-blur-md px-3 py-1.5 rounded-lg border border-white/10 shadow-sm">
                      <span className="text-[10px] font-bold text-tx-primary/40 mr-1">LNG</span>
                      <span className="text-[10px] font-medium text-tx-primary font-questrial">{Number(formData.longitude).toFixed(4)}</span>
                   </div>
                </div>
              </div>

              {/* Kolom Kanan: Inputs */}
              <div className="p-8 space-y-6 overflow-y-auto custom-scrollbar lg:border-l border-white/10">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  {/* Row 1: Name (Spans both columns) */}
                  <div className="md:col-span-2">
                    <label className="block text-sm font-gasoek font-normal tracking-wide text-tx-primary mb-1">Nama Komunitas *</label>
                    <input 
                      required type="text" name="communityname" value={formData.communityname} onChange={handleChange}
                  ...
                  <div className="md:col-span-2 space-y-3">
                    <label className="block text-sm font-gasoek font-normal tracking-wide text-tx-primary mb-1">Privasi Komunitas</label>
                    <div className="flex gap-4">
                      ...
                      >
                        ...
                        <div className="text-left leading-tight">
                          <p className="text-sm font-bold font-questrial">Publik</p>
                          <p className="text-[10px] font-medium opacity-60">Dapat ditemukan semua orang.</p>
                        </div>
                      </button>
                      <button 
                        type="button"
                      ...
                      >
                        <Lock size={20} />
                        <div className="text-left leading-tight">
                          <p className="text-sm font-bold font-questrial">Privat</p>
                          <p className="text-[10px] font-medium opacity-60">Hanya untuk undangan.</p>
                        </div>
                      </button>
                    </div>
                  </div>
                  ...
                  <div className="md:col-span-2 space-y-3">
                    <label className="block text-sm font-gasoek font-normal tracking-wide text-tx-primary mb-1">Foto Profil Komunitas</label>
                    <label className="w-full flex items-center justify-center gap-2 px-4 py-3 bg-bg-clean border border-dashed border-white/40 rounded-xl hover:bg-white/5 transition-colors cursor-pointer text-tx-primary/70 font-questrial text-sm">
                      ...
                      <input type="file" required={!formData.profilepicturl} accept="image/*" onChange={(e) => handleFileChange(e, 'profilepicturl')} className="hidden" />
                    </label>
                    ...
                    {formData.profilepicturl && (
                      <div className="p-4 bg-bg-clean/40 rounded-xl border border-white/10 flex flex-col gap-4 items-center">
                        <div className="aspect-square w-full md:w-32 rounded-lg overflow-hidden bg-bg-clean border border-white/10 shadow-inner shrink-0">
                            <img src={formData.profilepicturl} alt="Profile Preview" className="h-full w-full object-cover" 
                  ...
                  <div className="md:col-span-2 space-y-3">
                    <label className="block text-sm font-gasoek font-normal tracking-wide text-tx-primary mb-1">Foto Sampul Komunitas (Banner)</label>
                    <label className="w-full flex items-center justify-center gap-2 px-4 py-3 bg-bg-clean border border-dashed border-white/40 rounded-xl hover:bg-white/5 transition-colors cursor-pointer text-tx-primary/70 font-questrial text-sm">
                      <Upload size={18} />
                      <span>Pilih Foto Sampul</span>
                      <input type="file" required={!formData.bannerurl} accept="image/*" onChange={(e) => handleFileChange(e, 'bannerurl')} className="hidden" />
                    </label>
                  ...
                  <div className="md:col-span-2">
                    <label className="block text-sm font-gasoek font-normal tracking-wide text-tx-primary mb-1">Tentang Komunitas *</label>
                    <textarea 
        ...
        <div className="p-6 border-t border-white/10 bg-bg-vermillion text-tx-primary flex justify-end gap-3">
          <button 
            type="button" 
            onClick={onClose}
          ...
          >
            {isLoading ? (
              ...
              initialData ? 'Simpan Perubahan' : 'Buat Komunitas'
            )}
          </button>
        </div>
      </motion.div>
