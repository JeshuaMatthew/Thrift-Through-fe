...
export default function OrdersPage() {
  ...
  const activeTab = (searchParams.get("tab") === "purchases" ? "purchases" : "sales") as "sales" | "purchases";
    ...
    const runFetch = async () => {
      ...
      try {
        ...
        const result = activeTab === "sales"
          ? await transactionService.getTransactionsAsSeller(params)
          : await transactionService.getTransactionsAsBuyer(params);
  ...
  }, [user?.userid, authLoading, activeTab, currentPage, searchQuery, statusFilter, sortBy, order, refreshKey]);
        ...
        <div className="mb-10 space-y-4">
          <div className="flex flex-col md:flex-row gap-4 items-start md:items-center">
            ...
            <div className="flex flex-wrap items-center gap-4">
              <div className="flex items-center gap-2 bg-bg-vermillion/10 p-1.5 rounded-2xl border border-bg-vermillion/20">
                ...
                {[
                  ...
                  <button
                    ...
                    className={`px-3 py-1.5 rounded-xl text-xs font-gasoek transition-all flex items-center gap-1 ${sortBy === s.field
              ...
              <div className="relative flex items-center bg-bg-vermillion/10 p-1.5 rounded-2xl border border-bg-vermillion/20 h-[46px]">
                <Filter className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-tx-primary pointer-events-none" />
                <select
                  value={statusFilter}
                ...
                <div className="absolute right-4 top-1/2 -translate-y-1/2 pointer-events-none">
                  <ArrowUpDown size={12} className="text-tx-primary" />
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="flex gap-2 overflow-x-auto pb-1 no-scrollbar">
            <button
              ...
              className={`px-5 py-3 rounded-2xl text-sm tracking-wide whitespace-nowrap transition-all border font-gasoek ${activeTab === "sales"
            ...
            <button
              ...
              className={`px-5 py-3 rounded-2xl text-sm tracking-wide whitespace-nowrap transition-all border font-gasoek ${activeTab === "purchases"
                  ? "bg-bg-fresh border-bg-fresh/50 text-tx-primary shadow-md"
                  : "bg-bg-vermillion border-bg-vermillion/50 text-tx-primary hover:bg-bg-vermillion/80 shadow-sm"
                }`}
            >
            ...
            >
              {transactions.map((tx) => {
                ...
                const otherUser = {
                  ...
                  fullname: activeTab === "sales" ? tx.buyer_name : tx.seller_name,
                  username: activeTab === "sales" ? tx.buyer_user_name : tx.seller_user_name,
                  email: activeTab === "sales" ? tx.buyer_email : tx.seller_email,
                  profilepicturl: activeTab === "sales" ? tx.buyer_profile_pict : tx.seller_profile_pict,
                  phonenum: activeTab === "sales" ? tx.buyer_phone_num : tx.seller_phone_num,
                  bannerimgurl: activeTab === "sales" ? tx.buyer_banner_img : tx.seller_banner_img,
                };
                  ...
                  >
                    <div className="flex justify-between items-start mb-6">
                      <div className="flex flex-wrap items-center gap-2">
                        <div className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg border text-[10px] font-questrial uppercase tracking-wider ${statusConfig.className}`}>
                          {statusConfig.icon}
                          {statusConfig.label}
                        </div>
                        <div className="px-3 py-1.5 rounded-lg bg-bg-fresh/50 border border-bg-fresh/50 text-[10px] font-questrial uppercase tracking-wider text-tx-primary">
                          {tx.transactiontype}
                        </div>
                      </div>
                      <div className="text-right">
                        <p className="text-[9px] uppercase font-questrial text-tx-primary tracking-widest leading-none">ID Transaksi</p>
                        <p className="text-xs font-questrial text-tx-primary mt-1">#{tx.transactionid}</p>
                      </div>
                    </div>
                    ...
                    <div className="flex gap-4 mb-6">
                      ...
                      >
                        <img
                          src={tx.item_pict_url || "https://dummyimage.com/150x150/ddd/888&text=No+Image"}
                          alt={tx.item_name}
                          className="w-full h-full object-cover"
                        />
                      </div>
                      <div className="flex flex-col justify-center min-w-0">
                        <h3 className="text-lg font-gasoek text-tx-primary line-clamp-1 mb-1 leading-tight">{tx.item_name}</h3>
                        <p className="text-tx-secondary text-xs font-questrial mb-2">{tx.category}</p>
                      ...
                      <div className="flex flex-col sm:flex-row justify-between items-center gap-4">
                        ...
                        >
                          ...
                          <div className="min-w-0">
                            <p className="text-[9px] uppercase font-questrial text-tx-primary tracking-widest leading-none mb-0.5">{activeTab === "sales" ? "Pembeli" : "Penjual"}</p>
            ...
            <div className="flex items-center justify-start gap-2 flex-wrap order-1 md:order-1">
              ...
              {Array.from({ length: totalPages }, (_, i) => i + 1).map((page) => (
                <button
                  ...
                  className={`w-10 h-10 rounded-xl text-sm font-gasoek transition-all border ${currentPage === page
